...
def build_mip_optimization_model(n, solver_choice="PULP_CBC_CMD", with_symmetry=True):
    ...
    model = LpProblem(f"STS_Optimization_n{n}", LpMinimize)
    ...
    if with_symmetry:
        ...
        # Fix the first match: Team 1 (H) vs Team 2 (A) in Week 1, Period 1
        model += opp[0][0][1] == 1
        model += home[0][0] == 1
        model += home[0][1] == 0
    ...
    for w in range(W - 1):
        for t in Teams:
            # Method: breaks[w][t] = 1 if home[w][t] == home[w+1][t], 0 otherwise
            # For HH case (both 1): breaks[w][t] >= home[w][t] + home[w+1][t] - 1
            model += breaks[w][t] >= home[w][t] + home[w + 1][t] - 1
            
            # For AA case (both 0): breaks[w][t] >= 1 - home[w][t] - home[w+1][t]  
...
def solve_and_print(n, solver_choice="PULP_CBC_CMD", with_symmetry=True):
    # Track solving time to assess performance
    start = time.time()
    model, opp, home, period, solver = build_mip_optimization_model(n, solver_choice, with_symmetry)
    model.solve(solver)
...
if __name__ == "__main__":
    n = 10 # Adjust the number of teams here (must be even)
